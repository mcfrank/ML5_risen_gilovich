...
```{r}
( p.orig.main = p_orig( orig.y = yi.orig.main, orig.vy = vyi.orig.main,
...
```{r}
( p.orig.int = p_orig( orig.y = yi.orig.int, orig.vy = vyi.orig.int,
                      yr = Mhat, t2 = Vhat, vyr = SE.Mhat^2) )
```
...
# SEs = aggregate( Likelihood ~ tempt + Load, data = rpp, function(x) sqrt( var(x) / length(x) ) )

# # arguments are sample means, SDs, and 
# SMD = function( L0.T1, L0.T0, L1.T0, L1.T1,
#                 L0.T1.SD, L0.T0.SD, L1.T0.SD, L1.T1.SD,
#                 L0.T1.n, L0.T0.n, L1.T0.n, L1.T1.n ) {
#   
#   # main effect SMD
#   
#   aggregate( b$lkl )
